    ...
    Renderer: 'a+iced::advanced::Renderer,
{
    ...
    fn new(state: &'b mut Tree, overlays: &'b mut Vec<Element<'a, Message, Theme, Renderer>>, depth: impl Into<usize>) -> Self
    {
        ModalOverlay {
            state,
            overlays,
    ...
    Renderer: 'a+iced::advanced::Renderer,
{
    fn layout(&mut self, renderer: &Renderer, bounds: Size) -> Node {
        ...
        let mut underlay = self.overlays.get(self.depth).expect("Wrong depth.").as_widget()
            .layout(&mut self.state.children[self.depth + 1], renderer, &limits);
        let max_size = limits.max();

        underlay = underlay.align(Alignment::Center, Alignment::Center, max_size);
