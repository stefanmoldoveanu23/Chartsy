    ...
    Theme: StyleSheet,
{
    ...
    fn layout(&self, tree: &mut Tree, renderer: &Renderer, limits: &Limits) -> Node {
        ...
        let width = self.image.as_widget().size().width;
        let height = self.image.as_widget().size().height;
        ...
        let size = limits.resolve(width, height, image.size());

        image = image.move_to(Point::new(padding.left, padding.top));
        let image_size = image.size();
        image = image.align(Alignment::Center, Alignment::Center, image_size);
