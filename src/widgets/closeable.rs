    ...
    Theme: StyleSheet
{
    ...
    fn layout(&self, tree: &mut Tree, renderer: &Renderer, limits: &Limits) -> Node {
        ...
        let close_node = if let Some(close_button) = &self.close_button {
            let mut close_node = close_button.as_widget().layout(
                &mut tree.children[1],
                renderer,
                &limits
            );
            ...
            close_node = close_node.align(Alignment::End, Alignment::Start, size);
            close_node = close_node.move_to(Point::new(
        ...
        content_node = content_node.align(Alignment::Center, Alignment::Center, size);
